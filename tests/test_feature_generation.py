...
# Mock the imported functions to test independently
# Replace with actual import path once functions are available
@pytest.fixture
def mock_functions(monkeypatch):
    # Mock all functions to avoid import errors during testing
    ...
    def mock_calculate_experience_months(entries):
        if not entries:
            return []
        # Simple mock: return approximate months
        return [12, 24, 36]  # fixed values for testing
    ...
    def mock_clean_and_reduce_skills(df, column_name, threshold):
        def reduce_skills(skills):
            ...
            return list(set(skills_lower))  # unique lowercase
...
def test_calculate_experience_months():
    ...
    assert sum(months) > 30  # 12 + 12 + current ~40+ months

def test_generate_worker_features(simple_df, mock_functions):
    ...
    mask = simple_df["work_experience"].str.len() > 2
    assert (result.loc[mask, "count_works"] >= 1).all()

def test_extract_salaries(simple_df, mock_functions):
    ...
    assert non_zero.sum() >= 2  # At least two rows have salaries
...
def test_clean_and_reduce_skills(mock_functions):
    ...
    for skills_list in reduced:
        ...
        assert all(skill == skill.lower() for skill in skills_list)
...
def test_add_features_to_dataframe(mock_functions):
    ...
    assert "sql" in result.columns
    assert result["python"].iloc[0] == 1
    assert result["sql"].iloc[0] == 1
...
def test_read_features(mock_functions):
    ...
    assert "python" in [f.lower() for f in features]
